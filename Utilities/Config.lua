...
function Config:WriteJSON(Name,Table)
	if not isfolder("nullware") then makefolder("nullware") end
	if not isfolder("nullware/Configs") then makefolder("nullware/Configs") end
	writefile("nullware/Configs/"..Name..".json",HttpService:JSONEncode(Table))
end

function Config:ReadJSON(Name,Preset)
	if not isfile("nullware/Configs/"..Name..".json") then Config:WriteJSON(Name,Preset) return Preset end
	local DecodedJSON = HttpService:JSONDecode(readfile("nullware/Configs/"..Name..".json"))
